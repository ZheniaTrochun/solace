...
exports.createSession = function(buffer_size) {
    var session = new Decoder();
    session.open(buffer_size);
    return session;
};
...
module.exports.dump_bytes = function(raw_packet, offset) {
...
decode.packet = function(raw_packet) {
    var packet = {};
    // packet = decode.ethernet(raw_packet, 18);  
	packet = decode.ethernet(raw_packet, 0); 
    return packet;
};
...
module.exports.getrawtype = function(raw_packet, offset) {
    decode.rawtype(raw_packet, offset)
}
...
module.exports.decodeethernet = function(raw_packet, offset) { 
    return decode.ethernet(raw_packet, offset)
}
...
decode.udp = function(raw_packet, offset) {
    ...
    // 4, 5
    ret.checksum = unpack.uint16(raw_packet, offset + 6);
    // 6, 7
...
decode.tcp = function(raw_packet, offset, ip) {
    ...
    // second 4 bits of 12
    ret.flags = {};
    ret.flags.cwr = (raw_packet[offset + 13] & 128) >> 7;
...
decode.dns = function(raw_packet, offset) {
    ...
    ret.header = {};
    ret.header.id = unpack.uint16(raw_packet, offset);
    // 0, 1
    ...
    ret.header.qdcount = unpack.uint16(raw_packet, offset + 4);
    // 4, 5
    ret.header.ancount = unpack.uint16(raw_packet, offset + 6);
    // 6, 7
    ret.header.nscount = unpack.uint16(raw_packet, offset + 8);
    // 8, 9
    ret.header.arcount = unpack.uint16(raw_packet, offset + 10);
    // 10, 11
    ...
    while (pos < incoming_buf.length) {
        ...
        case "read_until_marker":
            if (incoming_buf[pos] !== 255) {
                ...
                // this gets converted to "websocket message" in TCP_Tracker
                this.state = "frame_type";
                this.buffer.end = 0;
                pos += 1;
            }
    ...
    if (packet.link && packet.link.ip && packet.link.ip.tcp) {
        ...
        if (tcp.flags.syn && !tcp.flags.ack) {
            if (session === undefined) {
                ...
                session.src_name = dns_cache.ptr(ip.saddr,
                function(name) {
                    session.src_name = name + ":" + tcp.sport;
                    self.emit("reverse", ip.saddr, name);
                }) + ":" + tcp.sport;
                session.dst_name = dns_cache.ptr(ip.daddr,
                function(name) {
